...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    for (int i = 0; i < h; ++i)
      for (int j = 0; j < w; ++j) {
        Spectrum t = envMap->data[i * w + j] * sin(PI * i / h);
        double s = t.x + t.y + t.z;
        pdf_envmap[i * w + j] = s;
        sum += s;
      }
    for (int i = 0; i < w * h; ++i)
    ...
    for (int i = 0; i < h; ++i) {
      marginal_y[i] = 0;
      for (int j = 0; j < w; ++j) {
        marginal_y[i] += pdf_envmap[i * w + j];
        conds_y[i * w + j] = pdf_envmap[i * w + j] + (j == 0 ? 0 : conds_y[i * w + j - 1]);
      }
      for (int j = 0; j < w; ++j)
        conds_y[i * w + j] /= marginal_y[i];
      marginal_y[i] += (i == 0 ? 0 : marginal_y[i - 1]);
    }
  ...
  Spectrum EnvironmentLight::sample_L(const Vector3D& p, Vector3D* wi,
    ...
    */
    ...
    int y = std::upper_bound(marginal_y, marginal_y + envMap->h - 1, s.y) - marginal_y;
    int x = std::upper_bound(conds_y + y * envMap->w, conds_y + (y + 1) * envMap->w - 1, s.x) - (conds_y + y * envMap->w);
    Vector2D theta_phi = xy_to_theta_phi(Vector2D(x + 0.5, y + 0.5));
    *wi = theta_phi_to_dir(theta_phi);
    *distToLight = INF_D;
    *pdf = pdf_envmap[y * envMap->w + x] * envMap->w * envMap->h / (2 * PI * PI * sin(theta_phi.x));
